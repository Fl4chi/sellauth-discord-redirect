...
DISCORD_CHANNEL_URL = os.getenv(
    "DISCORD_CHANNEL_URL",
    "https://discord.com/channels/1297953096273625098/1297958674882363432"
)
SELLAUTH_WEBHOOK_SECRET = os.getenv("SELLAUTH_WEBHOOK_SECRET", "")  # opzionale
...
# Inizializza il DB subito all'avvio (compatibile Flask 3.x)
init_db()
...
@app.get("/pay")
def pay_page():
    invoice_id = (request.args.get("invoice") or "").strip()
    status = None
    if invoice_id:
        conn = get_db()
    ...
    is_paid = (status in {"paid", "completed", "success"}) or (status is None)
